...
harm_pa_cl_ir <- function(adm_code_sel, param){
 ...
 # Synergy irrigated area
  ...
  cl1_df <- stack(grid, cl_raw, cl_rank_raw, cl_max_raw, grid_size) %>%
  ...
  # select adm1 if solve_sel = 1
  if(solve_sel == 1) {
    cl1_df <- cl1_df %>%
      filter(adm1_code == adm_code_sel)
  }
  
  # Check if cl1 > cl_max, probably caused by aggregation
  nrow(cl1_df %>%
         mutate(diff = cl_max - cl1) %>%
         filter(diff < 0))
  
  # Set cl_max to cl1 if cl > cl_max
  cl1_df <- cl1_df %>%
    mutate(cl_max = ifelse(cl1 > cl_max, cl1, cl_max))
  
  # Check if grid_size < cl_max
  check_cl_max <- cl1_df %>%
    mutate(diff = grid_size - cl_max) %>%
    filter(diff <0)
  hist(check_cl_max$diff)
  rm(check_cl_max)
  
  # Set cl_max to grid_size
  cl1_df <- cl1_df %>%
    mutate(cl_max = ifelse(grid_size < cl_max, grid_size, cl_max))
  ...
  cl2_df <- cl2_df %>%
    filter(!is.na(cl_rank2)) %>%
    dplyr::select(-cl_area, -cl_area_max)
  summary(cl2_df)
