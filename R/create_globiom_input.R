...
create_globiom_input <- function(mapping, lc_map, param) {
  ...
  simu_lc_df <- calc_lc_area(mapping, lc_map, param)
  ...
  simu_crop_df <- spam2simu("pa", param)
  ...
  simu_lc_upd <- merge_lc_crop(simu_lc_df, simu_crop_df)

  # Land cover gdx file
  create_land_cover_gdx(simu_lc_upd, param)

  # Crop distribution gdx file
  create_crop_distribution_gdx(simu_crop_df, param)
