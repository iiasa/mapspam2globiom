...
create_land_cover_gdx <- function(lc, param) {
  ...
  lc_gdx <-  para_gdx(lc, c("SimUID", "globiom_lc_code"),
                      "land_cover", "Updated land cover (000 ha)")
  ...
  gdxrrw::wgdx(file.path(temp_path, glue::glue("globiom_land_cover_{param$res}_{param$year}_{param$iso3c}")),
               lc_gdx)
