...
Potential revenue at the grid cell level is used to construct the scores/priors for the high-input and irrigated farming systems (see [Model description](model_description.html) for details). `00_prepare_faostat_crop_prices` extracts and prepares this using the FAOSTAT price database you downloaded before (see [Input data collection](input_data_collection)). As price data is often incomplete at the country level, we decided to use continental averaged prices instead. Make sure sure to set the `faostat_version` in the script, which corresponds to the date the FAOSTAT files were downloaded.
...
Finally, all the inputs, including the harmonized cropland extent, irrigated area extent and statistics, and the priors/scores are combined in one GAMS gdx file, which is used as input by SPAMc. 
