...
To install mapspam, which is not on CRAN, one first has to install the remotes package:
...
To interface between R and GAMS the GDXRRW package is required. This package is also not available on CRAN and therefore cannot be installed using the standard approach. One has to install it manually. The package can be found [here](https://support.gams.com/gdxrrw:interfacing_gams_and_r)
...
Several functions in mapspam2globiom use GDXRRW to read and save GAMS gdx files and check whether the package and GAMS are installed. If not, an error message is printed.
...
- [gdalUtils package](https://cran.r-project.org/web/packages/gdalUtils/index.html)[@ADD]
