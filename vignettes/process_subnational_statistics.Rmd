...
- `05_calculate_irrigated_area_share.r`

The first script prepares the national FAOSTAT statistics. These are used for two purposes. The first is for calibration of the subnational statistics so they are in line with FAOSTAT. As FAOSTAT is the leading source on agricultural data, it is useful to ensure that the subnational data adds up to the FAOSTAT national totals. Also, GLOBIOM is fully calibrated to FAOSTAT. For many countries the totals will already be the same or at least in the same range as subnational agricultural statistics are often the basis for construction national statistics, which in turn are used as input by FAOSTAT. However, for some countries there might be large differences. The second purpose is to determine the list of crops that is needed to prepare the subnational agricultural statistics. We will discuss this further below. Make sure to run `01_process_faostat_crops.r` before starting with preparing the subnational statistics. Do not forget to set the `faostat_version` variable in the script, which corresponds with the date you added when you downloaded the data (`20200303` in the Malawi example). 

The second and third scripts select relevant country data from the AQUASTAT database and use this to calculate the share of irrigated area per crop. This information can be used to as input to inform the irrigated area shares that are needed for the farming system shares data table (See below). We will not discuss these scripts here. Note that `05_calculate_irrigated_area_share.r` needs the processed harvest area statistics as input so it can only by run at the end, denoted by the number 05 in the name of the script. Please have a look and try by yourself. 
...
2. A file with the **Cropping intensity (ci)** for each subnational administrative unit at the national and subnational 1 (similar to the farming system shares).
3. A file with  **Farming system area shares (fs)** (not percentages!) for each subnational administrative unit at the national level. If the model is solved at the subnational level 1 (by setting the model level to 1), also farming system share data need to be provided for that level. 
...
Finally the three files (ha, fs and ci), need to be save in the `processed_data/agricultural_statistics folders. It is important to use the correct names otherwise other functions cannot find the data. 
...
Now most of the hard work is done, the next step is to [process the spatial data](process_spatial_data.html).
